...
class Mdl_Payments extends Response_Model
{
    ...
    {
       if($this->uri->segment(1) === 'suppliers'){
             $this->select("SQL_CALC_FOUND_ROWS
                        ip_payment_methods.*,
                        ip_invoice_amounts.*,
                        ip_suppliers.supplier_name,
                        ip_suppliers.supplier_surname,
                          ip_suppliers.supplier_id,
    ...
    public function default_join()
    {
        $this->db->join('ip_invoices', 'ip_invoices.invoice_id = ip_payments.invoice_id');
           if($this->uri->segment(1) === 'suppliers'){
              $this->db->join('ip_suppliers', 'ip_suppliers.supplier_id = ip_invoices.supplier_id');
           }else{
